delete_buttons = $(".discard_request");

function menage_request_item(delete_button, token){
    if(delete_button.value == "Убрать из корзины"){
        $.ajax({
            type:'POST',
            url: window.location.href.replace('shopping_cart', 'discard_request_from_cart'),
            data:
            {
        ...
        if(delete_button.value == "Вернуть в корзину"){
            $.ajax({
                type:'POST',
                url: window.location.href.replace('shopping_cart', 'add_request_to_cart'),
                data:
                {
                    csrfmiddlewaretoken: token,
                    id: $(delete_button).siblings("#id_CatalogItem_id").val(),
...
for (const delete_button of delete_buttons){
    ...
    delete_button.addEventListener("click", function() {menage_request_item(delete_button, token);});
}
